...
//                    ps.setInt(1, new productDAO().selectProduct(orderDetail.getProduct().getId()).getQuantity() - orderDetail.getQuantity());
    ...
    public ArrayList<Order> getAllOrder(User user){
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            while(rs.next()){
                a.add(new Order(rs.getInt("id"), rs.getString("order_date"), rs.getString("paymentMethod"), rs.getInt("state"), new OrderDAO().getOrderDetail(rs.getInt("id")), user));
            }
        } catch (Exception e) {
        }
        return a;
